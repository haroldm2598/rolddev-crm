...
export function useFetchBooks() {
  const {
    ...
    queryFn: async () => {
      const apiUrl =
        process.env.NODE_ENV === "development"
          ? "http://localhost:3000/api/book" // In development, use the full URL
          : "/api/book"; // For production, relative URL works fine
    ...
    refetchOnReconnect: false, // ✅ Added: refetch when user regains connection
    retry: 1,
  });

  return { data: bookData ?? [], error, refetch, isLoading, isFetching };
}

// ORIGINAL FOR USEFETCHBOOKS
// export function useFetchBooks() {
//     const {
//         data: bookData,
//         isLoading,
